...
labels = An optional array of strings that can contain a set of label and tags
			associated with the stream you want users to receive
...
videoports = JSON array with local ports for receiving video frames (only for rtp, and simulcasting)
	...
	"list" : [
		{
			...
			"labels" : "<labels of mountpoint #1, if any>",
		...
		},
		{
			...
			"labels" : "<labels of mountpoint #2, if any>",
			"audio_age_ms" : <how much time passed since we last received audio; optional, available for RTP mountpoints only>,
			"video_age_ms" : <how much time passed since we last received video; optional, available for RTP mountpoints only>
		},
		...
...
\verbatim
{
	...
	"info" : {
		...
		"labels" : "<labels of mountpoint, if any>",
...
gboolean janus_streaming_is_fd_there(int fd, int *fds, size_t fds_len);
...
static struct janus_json_parameter rtp_video_parameters[] = {
	...
	{"videoports", JSON_ARRAY, JANUS_JSON_PARAM_REQUIRED},
...
typedef struct janus_streaming_rtp_relay_packet_brutto {
	struct janus_streaming_rtp_relay_packet packet;
	uint32_t ssrcs[255];
} janus_streaming_rtp_relay_packet_brutto;
...
static uint32_t * janus_streaming_rtp_relay_packet_ssrcs(janus_streaming_rtp_relay_packet *pkt) {
	if(pkt == NULL || pkt == &exit_packet)
		return NULL;
	return (uint32_t *)&((janus_streaming_rtp_relay_packet_brutto *)pkt)->ssrcs;
}
static size_t janus_streaming_rtp_relay_packet_size(janus_streaming_rtp_relay_packet *pkt) {
	if(pkt == NULL || pkt == &exit_packet)
		return 0;
	return (size_t)(sizeof(janus_streaming_rtp_relay_packet) + (pkt->ssrcs_amount * sizeof(uint32_t)));
}

static void g_hash_table_remove_list(GHashTable *hash_table, GList *key_list) {
	if (NULL != key_list && NULL != hash_table) {
		for (guint idx=0; idx<g_list_length(key_list); idx++) {
			gpointer key = g_list_nth_data(key_list, idx);
			if (NULL != key) {
...
typedef struct janus_streaming_rtp_source {
	...
	janus_rtp_switching_context *contexts;
	int audio_fd;
	int *video_fds;
	int data_fd;
...
janus_streaming_mountpoint *janus_streaming_create_rtp_source(
		char *id_str, char *name, char *desc, GList *its_aliases, GList *labels, char *metadata,
		...
		gboolean dovideo, gboolean dovideortcp, char *vmcast, const janus_network_address *viface,
			uint16_t vrtcpport, uint8_t vcodec, char *vrtpmap, char *vfmtp, gboolean bufferkf,
			gboolean simulcast, uint8_t vports_length, uint16_t *vports, gboolean svc, gboolean dovskew, int rtp_collision,
...
janus_streaming_mountpoint *janus_streaming_create_file_source(
		char *id_str, char *name, char *desc, GList *its_aliases, GList *labels, char *metadata, char *filename, gboolean live,
...
typedef struct janus_streaming_session {
	...
	uint8_t last_spatial_layers_length;
	gint64 *last_spatial_layers;
...
static void janus_streaming_session_free(const janus_refcount *session_ref) {
	...
	g_free(session->last_spatial_layers);
	g_free(session);
}

int janus_streaming_session_free_last_spatial_layers(janus_streaming_session *session);
int janus_streaming_session_create_last_spatial_layers(janus_streaming_session *session, uint8_t len);

static gpointer	g_list_strdup(gconstpointer src, gpointer data) {
...
int janus_streaming_init(janus_callbacks *callback, const char *config_path) {
	...
	mountpoints = g_hash_table_new_full(g_str_hash, g_str_equal,
		(GDestroyNotify)g_free, (GDestroyNotify)janus_streaming_mountpoint_destroy);
	mountpoints_temp = g_hash_table_new_full(g_str_hash, g_str_equal,
		(GDestroyNotify)g_free, NULL);
	aliases = g_hash_table_new_full(g_str_hash, g_str_equal,
	...
	if(config != NULL) {
		...
		while(cl != NULL) {
			...
			if(id == NULL || id->value == NULL) {
				...
				if(g_hash_table_lookup(mountpoints, (gpointer)id->value) != NULL || g_hash_table_lookup(aliases, (gpointer)id->value) != NULL) {
					/* It does... */
			...
			janus_mutex_lock(&mountpoints_mutex);
			g_hash_table_insert(aliases, (gpointer)g_strdup(id->value), (gpointer)g_strdup(id->value));
			janus_mutex_unlock(&mountpoints_mutex);
			janus_config_array *alss = janus_config_get(config, cat, janus_config_type_array, "aliases");
			GList *its_aliases = NULL;
			if (NULL != alss) {
				if (NULL!=alss->list && !strcasecmp(alss->name, "aliases") && janus_config_type_array==alss->type) {
					for (guint idx=0; idx < g_list_length(alss->list); idx++) {
						...
						if (NULL!=item->value && NULL==item->name && janus_config_type_item==item->type) {
							its_aliases = g_list_append(its_aliases, g_strdup(item->value));
						}
					}
				}
				janus_mutex_lock(&mountpoints_mutex);
				gchar *exists = NULL;
				for (guint idx=0; idx<g_list_length(its_aliases); idx++) {
					gchar * alias = g_list_nth_data(its_aliases, idx);
					if(NULL != alias && g_hash_table_lookup(aliases, alias) != NULL) {
						exists = alias;
						break;
					}
				}
				if(NULL != exists) {
					janus_mutex_unlock(&mountpoints_mutex);
					JANUS_LOG(LOG_ERR, "Can't add the Streaming mountpoint '%s', ID alias '%s', already exists...\n",
						cat->name, exists);
					g_list_free_full(its_aliases, g_free);
					its_aliases = NULL;
					cl = cl->next;
					continue;
				}
				for (guint idx=0; idx<g_list_length(its_aliases); idx++) {
					gchar * alias = g_list_nth_data(its_aliases, idx);
					g_hash_table_insert(aliases, (gpointer)g_strdup(alias), (gpointer)g_strdup(id->value));
				}
				janus_mutex_unlock(&mountpoints_mutex);
			}
			if(!strcasecmp(type->value, "rtp")) {
				...
				janus_config_array *vports = janus_config_get(config, cat, janus_config_type_array, "videoports");
				...
				uint8_t video_ports_length = 0;
				uint16_t *video_ports = NULL;
				uint16_t video_rtcp_port = 0;
				if(dovideo &&
						(vports == NULL || vports->list == NULL ||
				...
				if(dovideo && vports != NULL && vports->list != NULL) {
					GList *item = vports->list;
					video_ports_length = g_list_length(item);
					video_ports = g_malloc0(sizeof(uint16_t) * video_ports_length);
					int i = 0;
					while(item && i < video_ports_length) {
						janus_config_item *port = (janus_config_item *)item->data;
						if(port && port->type == janus_config_type_item && port->name == NULL && port->value != NULL)
							if(janus_string_to_uint16(port->value, video_ports+i) < 0) {
								free(video_ports);
								video_ports = NULL;
								break;
							}
					...
					if(NULL == video_ports ) {
						JANUS_LOG(LOG_ERR, "Can't add 'rtp' mountpoint '%s', invalid video ports...\n", cat->name);
						cl = cl->next;
						continue;
					}
				}
				...
				if (NULL != lbls) {
					if (NULL!=lbls->list && !strcasecmp(lbls->name, "labels") && janus_config_type_array==lbls->type) {
						for (guint idx=0; idx < g_list_length(lbls->list); idx++) {
							janus_config_item *item = g_list_nth_data(lbls->list, idx);
							if (NULL!=item->value && NULL==item->name && janus_config_type_item==item->type) {
								labels = g_list_append(labels, g_strdup(item->value));
							}
						}
					}
...
void janus_streaming_create_session(janus_plugin_session *handle, int *error) {
	...
	janus_streaming_session_free_last_spatial_layers(session);
...
json_t *janus_streaming_query_session(janus_plugin_session *handle) {
	...
	if(mp) {
		...
		json_object_set_new(info, "mountpoint_id", json_string(mp->id_str));
...
static json_t *janus_streaming_process_synchronous_request(janus_streaming_session *session, json_t *message) {
	...
	if(!strcasecmp(request_text, "list")) {
		...
		while(g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			json_object_set_new(ml, "id", json_string(mp->id_str));
			...
			if(mp->labels && 0 < g_list_length(mp->labels)) {
				...
				for (guint idx = 0; idx < g_list_length(mp->labels); idx++) {
					json_array_append(arr, json_string(g_list_nth_data(mp->labels, idx)));
				}
				json_object_set_new(ml, "labels", arr);
			}
			if(mp->metadata) {
		...
		json_t *alias = json_object_get(root, "id");
		char *alias_value_str = (char *)json_string_value(alias);
		janus_mutex_lock(&mountpoints_mutex);
		char *id_value_str = (char *)g_hash_table_lookup(aliases, (gpointer)alias_value_str);
		if (NULL == id_value_str) {
			janus_mutex_unlock(&mountpoints_mutex);
			JANUS_LOG(LOG_VERB, "No such alias '%s'\n", alias_value_str);
			error_code = JANUS_STREAMING_ERROR_NO_SUCH_MOUNTPOINT;
			g_snprintf(error_cause, 512, "No such alias '%s'", alias_value_str);
			goto prepare_response;
		}
		janus_streaming_mountpoint *mp = g_hash_table_lookup(mountpoints,
			(gpointer)id_value_str);
		if(mp == NULL) {
			janus_mutex_unlock(&mountpoints_mutex);
			JANUS_LOG(LOG_VERB, "No such mountpoint/stream '%s', alias '%s'\n", id_value_str, alias_value_str);
			error_code = JANUS_STREAMING_ERROR_NO_SUCH_MOUNTPOINT;
			g_snprintf(error_cause, 512, "No such mountpoint/stream '%s', alias '%s'", id_value_str, alias_value_str);
			goto prepare_response;
		}
		janus_refcount_increase(&mp->ref);
		...
		if(mp->streaming_source == janus_streaming_source_file) {
			...
			if(admin) {
				...
				if(mp->video) {
					...
					json_object_set_new(ml, "videoports", arr);
		...
		if(mpid_str != NULL) {
			...
			if(g_hash_table_lookup(mountpoints, (gpointer)mpid_str) != NULL ||
					g_hash_table_lookup(mountpoints_temp, (gpointer)mpid_str) != NULL) {
				janus_mutex_unlock(&mountpoints_mutex);
				JANUS_LOG(LOG_ERR, "A stream with the provided ID '%s' (alias: '%s') already exists\n", mpid_str, alias_value_str);
		...
		g_hash_table_insert(aliases, (gpointer)g_strdup(mpid_str), (gpointer)g_strdup(mpid_str));
		json_t *its_aliases = json_object_get(root, "aliases");
		GList * alss = NULL;
		for (size_t idx=0; idx<json_array_size(its_aliases); idx++) {
			alss = g_list_append(alss, (gpointer)json_string_value(json_array_get(its_aliases, idx)));
		}
		for (guint idx=0; idx<g_list_length(alss); idx++) {
			gchar *alias = g_list_nth_data(alss, idx);
			if (NULL != alias && NULL != g_hash_table_lookup(aliases, (gpointer)alias)) {
				g_hash_table_remove(aliases, (gpointer)mpid_str);
				g_hash_table_remove_list(aliases, alss);
				janus_mutex_unlock(&mountpoints_mutex);
				JANUS_LOG(LOG_ERR, "A stream with the provided alias '%s' already exists\n", alias);
				error_code = JANUS_STREAMING_ERROR_CANT_CREATE;
		...
		if(!strcasecmp(type_text, "rtp")) {
			...
			if(error_code != 0) {
				janus_mutex_lock(&mountpoints_mutex);
				g_hash_table_remove(mountpoints_temp, (gpointer)mpid_str);
				g_hash_table_remove(aliases, (gpointer)mpid_str);
				g_hash_table_remove_list(aliases, alss);
				janus_mutex_unlock(&mountpoints_mutex);
				goto prepare_response;
			...
			uint8_t vports_length = 0;
			uint16_t *vports = NULL;
			...
			if(dovideo) {
				...
				json_t *videoports = json_object_get(root, "videoports");
				if (videoports) {
					if (UINT8_MAX <= json_array_size(videoports)) {
						JANUS_LOG(LOG_FATAL, "\"videoports\" array is to long. %lu >= %d.\n", json_array_size(videoports), UINT8_MAX);
						error_code = JANUS_STREAMING_ERROR_INVALID_ELEMENT;
						g_snprintf(error_cause, 512, "\"videoports\" array is to long.");
						janus_mutex_lock(&mountpoints_mutex);
						g_hash_table_remove(mountpoints_temp, (gpointer)mpid_str);
						g_hash_table_remove(aliases, (gpointer)mpid_str);
						g_hash_table_remove_list(aliases, alss);
					...
					vports_length = json_array_size(videoports);
					vports = (uint16_t *)g_malloc0(sizeof(uint16_t) * vports_length);
					for (uint8_t i=0; i < vports_length; i++) {
						json_t * tmp_vport = json_array_get(videoports, i);
						if (tmp_vport) {
							vports[i] = json_integer_value(tmp_vport);
						}
					}
				}
			...
			mp = janus_streaming_create_rtp_source(
					...
					dovideo, dovideortcp, vmcast, &video_iface, vrtcpport, vcodec, vrtpmap, vfmtp, bufferkf,
					simulcast, vports_length, vports, dosvc, dovskew,
			...
			if(!audiofile) {
				...
				error_code = JANUS_STREAMING_ERROR_CANT_CREATE;
				g_snprintf(error_cause, 512, "Can't add 'live' stream, no such file '%s'", filename);
				janus_mutex_lock(&mountpoints_mutex);
				g_hash_table_remove(mountpoints_temp, (gpointer)mpid_str);
				g_hash_table_remove(aliases, (gpointer)mpid_str);
				g_hash_table_remove_list(aliases, alss);
			...
			if(!audiofile) {
				...
				error_code = JANUS_STREAMING_ERROR_CANT_CREATE;
				g_snprintf(error_cause, 512, "Can't add 'ondemand' stream, no such file '%s'", filename);
				janus_mutex_lock(&mountpoints_mutex);
				g_hash_table_remove(mountpoints_temp, (gpointer)mpid_str);
				g_hash_table_remove(aliases, (gpointer)mpid_str);
				g_hash_table_remove_list(aliases, alss);
			...
			error_code = JANUS_STREAMING_ERROR_INVALID_ELEMENT;
			g_snprintf(error_cause, 512, "Unknown stream type '%s'...", type_text);
			goto prepare_response;
		}
		/* Any secret? */
		if(secret)
		...
		if(save) {
			...
			if (mp->labels && 0 < g_list_length(mp->labels)) {
				...
				janus_config_add(config, c, gl);
				for(guint idx=0; idx<g_list_length(mp->labels); idx++) {
					janus_config_add(config, gl, janus_config_item_create(NULL, (char *)g_list_nth_data(mp->labels, idx)));
				}
			}
			if(mp->metadata)
		...
		if(!strcasecmp(type_text, "rtp")) {
			...
			json_object_set_new(ml, "video_ports", arr);
			if(source->video_host)
		...
		if(notify_events && gateway->events_is_enabled()) {
			...
			json_object_set_new(info, "id", json_string(mp->id_str));
		...
		json_t *j_aliases = json_object_get(root, "new_aliases");
		...
		if(mp == NULL) {
			janus_mutex_unlock(&mountpoints_mutex);
			JANUS_LOG(LOG_ERR, "No such mountpoint '%s', alias '%s'\n", id_value_str, alias_value_str);
			error_code = JANUS_STREAMING_ERROR_NO_SUCH_MOUNTPOINT;
			g_snprintf(error_cause, 512, "No such mountpoint '%s', alias '%s'", id_value_str, alias_value_str);
			goto prepare_response;
		}
		janus_refcount_increase(&mp->ref);
		janus_mutex_lock(&mp->mutex);
		...
		if(aliases != NULL) {
			...
			for (guint idx=0; idx<g_list_length(new_aliases); idx++) {
				...
				if (NULL != alias && NULL == g_list_find(old_aliases, alias) && NULL != g_hash_table_lookup(aliases, alias)) {
					janus_mutex_unlock(&mp->mutex);
					janus_mutex_unlock(&mountpoints_mutex);
					janus_refcount_decrease(&mp->ref);
					JANUS_LOG(LOG_ERR, "Alias '%s' already exists\n", alias);
					error_code = JANUS_STREAMING_ERROR_NO_SUCH_MOUNTPOINT;
		...
		json_object_set_new(response, "id", json_string(mp->id_str));
		json_object_set_new(response, "permanent", save ? json_true() : json_false());
		/* Also notify event handlers */
		if(notify_events && gateway->events_is_enabled()) {
			json_t *info = json_object();
		...
		JANUS_LOG(LOG_VERB, "Request to unmount mountpoint/stream '%s', alias '%s'\n", id_value_str, alias_value_str);
		/* Remove mountpoint from the hashtable: this will get it destroyed eventually */
		g_hash_table_remove(mountpoints,
			(gpointer)id_value_str);
		janus_mutex_lock(&mp->mutex);
		g_hash_table_remove(aliases, (gpointer)id_value_str);
		g_hash_table_remove_list(aliases, mp->aliases);
		...
		while(viewer) {
			...
			s->mountpoint = NULL;
			janus_streaming_session_free_last_spatial_layers(s);
		...
		if(notify_events && gateway->events_is_enabled()) {
			...
			json_object_set_new(info, "id", json_string(id_value_str));
			gateway->notify_event(&janus_streaming_plugin, session ? session->handle : NULL, info);
		}
		janus_mutex_unlock(&mountpoints_mutex);
		/* Send info back */
		...
		json_object_set_new(response, "destroyed", json_string(id_value_str));
		goto prepare_response;
...
static void janus_streaming_hangup_media_internal(janus_plugin_session *handle) {
	...
	for(uint8_t i=0; i<session->last_spatial_layers_length; i++) {
		session->last_spatial_layers[i] = 0;
	}
...
static void *janus_streaming_handler(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(!strcasecmp(request_text, "watch")) {
			...
			JANUS_LOG(LOG_VERB, "Request to watch mountpoint/stream '%s', alias '%s'\n", id_value_str, alias_value_str);
			if(g_list_find(mp->viewers, session) != NULL) {
				janus_mutex_unlock(&session->mutex);
				janus_mutex_unlock(&mp->mutex);
				janus_refcount_decrease(&mp->ref);
			...
			janus_streaming_session_create_last_spatial_layers(session, mp->videos_amount);
			...
			if(notify_events && gateway->events_is_enabled()) {
				...
				if(session->mountpoint != NULL)
					json_object_set_new(info, "id", json_string(session->mountpoint->id_str));
			...
			json_object_set_new(result, "id", json_string(id_value_str));
			/* Also notify event handlers */
			if(notify_events && gateway->events_is_enabled()) {
				json_t *info = json_object();
...
janus_streaming_mountpoint *janus_streaming_create_rtp_source(
	...
	if(!doaudio && !dovideo && !dodata) {
		JANUS_LOG(LOG_ERR, "Can't add 'rtp' stream, no audio, video or data have to be streamed...\n");
		janus_mutex_lock(&mountpoints_mutex);
		g_hash_table_remove(mountpoints_temp, id_str);
		g_hash_table_remove(aliases, (gpointer)id_str);
		g_hash_table_remove_list(aliases, its_aliases);
		janus_mutex_unlock(&mountpoints_mutex);
		return NULL;
	}
	...
	int *video_fds = g_malloc0(sizeof(int) * vports_length);
	for (uint8_t i=0; i<vports_length; i++) {
		video_fds[i] = -1;
	}
	int video_rtcp_fd = -1;
	...
	videohost[0] = '\0';
	if(dovideo) {
		video_fds[0] = janus_streaming_create_fd(vports[0], vmcast ? inet_addr(vmcast) : INADDR_ANY, viface,
			videohost, sizeof(videohost), "Video", "video", name ? name : tempname, vports[0] == 0);
		if(video_fds[0] < 0) {
		...
		vports[0] = janus_streaming_get_fd_port(video_fds[0]);
		if(dovideortcp) {
		...
		if(simulcast) {
			for (uint8_t i=1; i<vports_length; i++) {
				video_fds[i] = janus_streaming_create_fd(vports[i], vmcast ? inet_addr(vmcast) : INADDR_ANY, viface,
					NULL, 0, "Video", "video", name ? name : tempname, FALSE);
				if(video_fds[i] < 0) {
					JANUS_LOG(LOG_ERR, "Can't bind to port %d for video (port number %d)...\n", vports[i], i);
					if(audio_fd > -1)
						close(audio_fd);
					if(audio_rtcp_fd > -1)
						close(audio_rtcp_fd);
				...
				vports[i] = janus_streaming_get_fd_port(video_fds[i]);
			}
		}
	}
	int data_fd = -1;
	...
	live_rtp->id_num = g_str_hash(id_str);
	...
	live_rtp->aliases = ((its_aliases && 0 < g_list_length(its_aliases)) ? g_list_copy_deep(its_aliases, g_list_strdup, NULL) : NULL);
	...
	live_rtp_source->video_ports_length = vports_length;
	live_rtp_source->video_ports = g_malloc0(sizeof(gint) * live_rtp_source->video_ports_length);
	live_rtp_source->video_ports[0] = dovideo ? vports[0] : -1;
	live_rtp_source->video_rtcp_port = vrtcpport;
	live_rtp_source->simulcast = dovideo && simulcast;
	if (live_rtp_source->simulcast) {
		for(uint8_t i=1; i<live_rtp_source->video_ports_length; i++) {
			live_rtp_source->video_ports[i] = vports[i];
		}
	} else {
		for(uint8_t i=1; i<live_rtp_source->video_ports_length; i++) {
			live_rtp_source->video_ports[i] = -1;
		}
	}
	...
	live_rtp_source->contexts = g_malloc0(sizeof(janus_rtp_switching_context) * vports_length);
	for(int i=0; i<vports_length; i++) {
		janus_rtp_switching_context_reset(&live_rtp_source->contexts[i]);
	}
	...
	live_rtp_source->video_fds = g_malloc0(sizeof(int) * vports_length);
	for(uint8_t i=0; i<vports_length; i++) {
		live_rtp_source->video_fds[i] = video_fds[i];
	}
	...
	g_hash_table_insert(mountpoints,
		(gpointer)g_strdup(live_rtp->id_str),
		live_rtp);
	g_hash_table_remove(mountpoints_temp, (gpointer)live_rtp->id_str);
	...
	if(threads > 0) {
		...
		for(i=0; i<threads; i++) {
			...
			g_snprintf(tname, sizeof(tname), "help %u-%s", helper->id, live_rtp->id_str);
...
janus_streaming_mountpoint *janus_streaming_create_file_source(
	...
	file_source->id_num = g_str_hash(id_str);
	char tempname[255];
	if(!name) {
		memset(tempname, 0, 255);
	...
	file_source->aliases = (its_aliases ? g_list_copy_deep(its_aliases, g_list_strdup, NULL) : NULL);
	...
	janus_mutex_lock(&mountpoints_mutex);
	g_hash_table_insert(mountpoints,
		(gpointer)g_strdup(file_source->id_str),
		file_source);
	g_hash_table_remove(mountpoints_temp, (gpointer)file_source->id_str);
	janus_mutex_unlock(&mountpoints_mutex);
	if(live) {
		GError *error = NULL;
		char tname[16];
...
static int janus_streaming_rtsp_connect_to_server(janus_streaming_mountpoint *mp) {
	...
	*curl_errbuf = '\0';
	...
	source->video_fds[0] = video_fds.fd;
...
static int janus_streaming_rtsp_play(janus_streaming_rtp_source *source) {
	...
	if(source->remote_video_port > 0 && source->video_fds[0] >= 0) {
		JANUS_LOG(LOG_VERB, "RTSP video latching: %s:%d\n", source->rtsp_vhost, source->remote_video_port);
		janus_streaming_rtsp_latch(source->video_fds[0], source->rtsp_vhost,
...
janus_streaming_mountpoint *janus_streaming_create_rtsp_source(
	...
	live_rtsp->aliases = (its_aliases ? g_list_copy_deep(its_aliases, g_list_strdup, NULL) : NULL);
	...
	live_rtsp_source->video_fds = g_malloc0(sizeof(int) * live_rtsp_source->video_ports_length);
	for(uint8_t i=0; i<live_rtsp_source->video_ports_length; i++) {
		live_rtsp_source->video_fds[i] = -1;
	}
	...
	if(threads > 0) {
		...
		for(i=0; i<threads; i++) {
			...
			/* Spawn a thread and add references */
			g_snprintf(tname, sizeof(tname), "help %u-%s", helper->id, live_rtsp->id_str);
	...
	janus_mutex_lock(&mountpoints_mutex);
	g_hash_table_insert(mountpoints,
		(gpointer)g_strdup(live_rtsp->id_str),
		live_rtsp);
	g_hash_table_remove(mountpoints_temp, (gpointer)live_rtsp->id_str);
	janus_mutex_unlock(&mountpoints_mutex);
	return live_rtsp;
}
#else
...
static void *janus_streaming_relay_thread(void *data) {
	...
	int *video_fds = g_malloc0(sizeof(int) * source->video_ports_length);
	for(uint8_t i=0; i<source->video_ports_length; i++) {
		video_fds[i] = source->video_fds[i];
	}
	...
	uint32_t ssrc = 0, a_last_ssrc = 0;
	uint32_t *v_last_ssrcs = g_malloc0(sizeof(uint32_t) * source->video_ports_length);
	/* File descriptors */
	socklen_t addrlen;
	struct sockaddr_storage remote;
	int resfd = 0, bytes = 0;
	struct pollfd *fds = g_malloc0(sizeof(struct pollfd) * (5 + source->video_ports_length));
	char buffer[1500];
	...
	/* Loop */
	int num = 0;
	janus_streaming_rtp_relay_packet_brutto packet_brutto;
	memset(&packet_brutto, 0, sizeof(janus_streaming_rtp_relay_packet_brutto));
	janus_streaming_rtp_relay_packet *packet = (janus_streaming_rtp_relay_packet *)&packet_brutto;
	packet->ssrcs_amount = mountpoint->videos_amount;
	while(!g_atomic_int_get(&stopping) && !g_atomic_int_get(&mountpoint->destroyed)) {
#ifdef HAVE_LIBCURL
			...
			if(!source->reconnecting && (now - source->reconnect_timer > source->reconnect_delay)) {
				...
				if(janus_streaming_rtsp_connect_to_server(mountpoint) < 0) {
					...
					if(janus_streaming_rtsp_play(source) < 0) {
						...
						video_fds[0] = source->video_fds[0];
		...
		for(i=0; i<num; i++) {
			if(fds[i].revents & (POLLERR | POLLHUP)) {
				...
				if(pipe_fd != -1 && fds[i].fd == pipe_fd) {
					...
					/* Is there a recorder? */
					janus_rtp_header_update(packet->data, &source->contexts[0], FALSE, 0);
					if(source->askew) {
						int ret = janus_rtp_skew_compensate_audio(packet->data, &source->contexts[0], now);
						if(ret < 0) {
					...
					if(source->arc) {
						packet->data->ssrc = htonl((uint32_t)mountpoint->id_num);
					...
					continue;
				} else if(janus_streaming_is_fd_there(fds[i].fd, video_fds, mountpoint->videos_amount)) {
					/* Got something video (RTP) */
					int index = -1;
					for(uint8_t fd_idx=0; fd_idx<mountpoint->videos_amount; fd_idx++) {
						if(fds[i].fd == video_fds[fd_idx]) {
							index = fd_idx;
							break;
						}
					}
					...
					janus_rtp_header *rtp = (janus_rtp_header *)buffer;
					ssrc = ntohl(rtp->ssrc);
					if(source->rtp_collision > 0 && v_last_ssrcs[index] && ssrc != v_last_ssrcs[index] &&
					...
					/* Do we have a new stream? */
					if(ssrc != v_last_ssrcs[index]) {
						v_last_ssrcs[index] = ssrc;
						if(index == 0)
							source->video_ssrc = ssrc;
						JANUS_LOG(LOG_INFO, "[%s] New video stream! (ssrc=%"SCNu32", index %d)\n",
							name, v_last_ssrcs[index], index);
					}
					/* Is this SRTP? */
					if(source->is_srtp) {
						int buflen = bytes;
					...
					janus_rtp_header_update(packet->data, &source->contexts[index], TRUE, 0);
					if(source->vskew) {
						int ret = janus_rtp_skew_compensate_video(packet->data, &source->contexts[index], now);
						if(ret < 0) {
							JANUS_LOG(LOG_WARN, "[%s] Dropping %d packets, video source clock is too fast (ssrc=%"SCNu32", index %d)\n",
								name, -ret, v_last_ssrcs[index], index);
							continue;
						} else if(ret > 0) {
					...
					if (mountpoint->enabled) {
						...
						uint32_t *ssrcs = janus_streaming_rtp_relay_packet_ssrcs(packet);
						if(source->simulcast) {
							for(uint8_t i=0; i<source->video_ports_length; i++) {
								ssrcs[i] = v_last_ssrcs[i];
							}
						}
						/* Go! */
						janus_mutex_lock(&mountpoint->mutex);
		...
		if(packet->is_video) {
			...
			if(packet->svc) {
				...
				if(packet->svc_info.spatial_layer >= 0 && packet->svc_info.spatial_layer <= 2)
					session->last_spatial_layers[packet->svc_info.spatial_layer] = now;
				if(session->target_spatial_layer > session->spatial_layer) {
					...
					if(keyframe) {
						...
						while(new_spatial_layer > session->spatial_layer && new_spatial_layer > 0) {
							if(now - session->last_spatial_layers[new_spatial_layer] >= 250000) {
				...
				gboolean relay = janus_rtp_simulcasting_context_process_rtp(&session->sim_context,
					(char *)packet->data, packet->length, packet->ssrcs_amount, janus_streaming_rtp_relay_packet_ssrcs(packet), NULL, packet->codec, &session->context);
				if(!relay) {
...
static void janus_streaming_helper_rtprtcp_packet(gpointer data, gpointer user_data) {
	...
	janus_streaming_rtp_relay_packet *copy = g_malloc0(janus_streaming_rtp_relay_packet_size(packet));
	...
	copy->ssrcs_amount = packet->ssrcs_amount;
	uint32_t *ssrcs_src = janus_streaming_rtp_relay_packet_ssrcs(packet);
	uint32_t *ssrcs_dst = janus_streaming_rtp_relay_packet_ssrcs(copy);
	for(uint8_t i=0; i<packet->ssrcs_amount; i++) {
		ssrcs_dst[i] = ssrcs_src[i];
	}
...
int janus_streaming_session_free_last_spatial_layers(janus_streaming_session *session) {
	if (NULL == session || NULL == session->last_spatial_layers) {
		return -1;
	}
	g_free(session->last_spatial_layers);
	session->last_spatial_layers = NULL;
	session->last_spatial_layers_length = 0;
	return 0;
}

int janus_streaming_session_create_last_spatial_layers(janus_streaming_session *session, uint8_t len) {
	if (NULL == session || NULL != session->last_spatial_layers || UINT8_MAX <= len) {
		return -1;
	}
	session->last_spatial_layers = g_malloc0(sizeof(gint64) * len);
	if (NULL == session->last_spatial_layers) {
		return -1;
	}
	session->last_spatial_layers_length = len;
	return 0;
}
