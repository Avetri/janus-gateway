...
// TODO: Replace janus_video_mediainfo * janus_streaming_rtp_source_stream::video_mediainfo by a GList
...
janus_streaming_mountpoint *janus_streaming_create_rtp_source(
		char *id_str, char *name, char *desc, GList *its_aliases, char *metadata,
...
janus_streaming_mountpoint *janus_streaming_create_file_source(
		char *id_str, char *name, char *desc, GList *its_aliases, char *metadata, char *filename, gboolean live,
...
static json_t *janus_streaming_process_synchronous_request(janus_streaming_session *session, json_t *message) {
	...
	if(!strcasecmp(request_text, "list")) {
		...
		while(g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			if(mp->streaming_source == janus_streaming_source_rtp) {
				...
				while(temp) {
					...
					if(stream->video_mediainfo) {
						char * mi = json_streaming_mediainfos_string(stream->video_mediainfo, (stream->simulcast)?3:1);
		...
		janus_mutex_lock(&mountpoints_mutex);
		char *mpid_str = (alias_value_str)?(char *)g_hash_table_lookup(aliases, (gpointer)alias_value_str):NULL;
		if(mpid_str != NULL) {
			...
			if(g_hash_table_lookup(mountpoints, (gpointer)mpid_str) != NULL ||
				...
				janus_mutex_unlock(&mountpoints_mutex);
				JANUS_LOG(LOG_ERR, "A stream with the ID '%s' (provided alias: '%s') already exists\n", mpid_str, alias_value_str);
				error_code = JANUS_STREAMING_ERROR_CANT_CREATE;
				g_snprintf(error_cause, 512, "A stream with the ID '%s' (provided alias: '%s') already exists", mpid_str, alias_value_str);
				goto prepare_response;
			}
		} else if(NULL != alias_value_str) {
			mpid_str = alias_value_str;
		} else {
		...
		for (guint idx=0; idx<g_list_length(alss); idx++) {
			...
			if (NULL != alias) {
				if (NULL != g_hash_table_lookup(aliases, (gpointer)alias)) {
...
janus_streaming_rtp_source_stream *janus_streaming_create_rtp_source_stream(
	...
	if (mtype == JANUS_STREAMING_MEDIA_VIDEO) {
		if (simulcast) {
			stream->video_mediainfo = g_malloc0(sizeof(janus_video_mediainfo) * 3);
		} else {
			stream->video_mediainfo = g_malloc0(sizeof(janus_video_mediainfo));
		}
	}
...
janus_streaming_mountpoint *janus_streaming_create_rtsp_source(
	...
	live_rtsp->id_num = g_str_hash(id_str);
...
/* Helper to create an RTSP source */
janus_streaming_mountpoint *janus_streaming_create_rtsp_source(
		uint64_t id, char *id_str, char *name, char *desc, GList *aliases, char *metadata,
		char *url, char *username, char *password,
		gboolean quirk, gboolean notify_changes,
			...
			if(!source->reconnecting && (now - source->reconnect_timer > source->reconnect_delay)) {
				...
				if(connected) {
					...
					if(notify_events && gateway->events_is_enabled()) {
						...
						json_object_set_new(info, "id", json_string(mountpoint->id_str));
		...
		for(i=0; i<num; i++) {
			if(fds[i].revents & (POLLERR | POLLHUP)) {
				...
				if(stream->type == JANUS_STREAMING_MEDIA_AUDIO && fds[i].fd == stream->fd[0]) {
					...
					/* Get mediainfo for RTP video stream */
					char *payload = NULL;
					int plen = 0;
					if (JANUS_VIDEOCODEC_H264 == stream->codecs.video_codec && (payload = janus_rtp_payload(buffer, bytes, &plen))) {
						...
						if(janus_h264_is_keyframe_n_mi(payload, plen, &mi) && 0 < mi.width && 0 < mi.height) {
							mi.port = stream->port[index];
							if(memcmp(&mi, stream->video_mediainfo+index, sizeof(janus_video_mediainfo))) {
								memcpy(stream->video_mediainfo+index, &mi, sizeof(janus_video_mediainfo));
					...
					if(stream->h264_spspps) {
						...
						if(payload && janus_h264_is_i_frame(payload, plen)) {
							...
							sps_rtp->ssrc = htonl((uint32_t)mountpoint->id_num);
